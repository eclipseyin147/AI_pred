...
namespace tju_torch {
    ...
    void FaultDiagManager::run(const nlohmann::json &config) {
        ...
        // Match the legacy TCN pipeline exactly: always use rescale-symmetric normalization.
        SequenceNormMethod norm_method = SequenceNormMethod::RESCALE_SYMMETRIC;

        // Keep legacy training behavior by default: no IPC controller unless explicitly enabled.
        bool enable_ipc = config.value("enable_ipc", true);
        std::unique_ptr<TrainingController> controller;
        if (enable_ipc) {
            controller = std::make_unique<TrainingController>(control_file, status_file);
        }
        ...
        if (submode == "test") {
            ...
            cnn_config.normalize_input = false; // CNN: no input normalization (matches m1DCNN.m)
            ...
            cnn_trainer.train(train_sequences, train_labels, {}, {}, controller.get());
            ...
            tcn_trainer.train(train_sequences, train_labels, {}, {}, controller.get());
        ...
        if (submode == "cnn") {
            ...
            train_config.normalize_input = false; // CNN: no input normalization (matches m1DCNN.m)
            SequenceTrainer<Conv1DNet> trainer(model, train_config);
            trainer.train(train_sequences, train_labels, val_sequences, val_labels, controller.get());
            ...
            trainer.predict(val_sequences, predictions);
            ...
            train_config.normalize_input = true; // TCN: rescale-symmetric normalization (matches myselfTCN.m)
            // Workaround: per-epoch validation with large validation sets (>~500 samples)
            // triggers an obscure stack-corruption crash inside LibTorch on MSVC/RelWithDebInfo.
            // Final evaluation after training completes runs correctly, so we skip
            // intermediate validation and only evaluate once at the end.
            train_config.validation_frequency = epochs + 1;
